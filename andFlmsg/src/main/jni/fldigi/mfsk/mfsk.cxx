...
//Android debug: with filter the output level is zero
//bool xmt_filter = true;
bool xmt_filter = false;
...
//Android void  mfsk::tx_init(SoundBase *sc)
void  mfsk::tx_init()
{
//Android 	scard = sc;
...
void mfsk::init()
{
	modem::init();
	rx_init();
//Android 	set_scope_mode(Digiscope::SCOPE);
// picture mode init
...
//Android 	if (picTxWin) picTxWin->hide();
//Android	activate_mfsk_image_item(false);
...
//Android mfsk::mfsk(trx_mode mfsk_mode) : modem()
mfsk::mfsk(int mfsk_mode) : modem()
{
	cap |= CAP_AFC | CAP_REV;
...
//Android	scopedata.alloc(symlen * 2);
...
//Android	if (!picTxWin) createTxViewer();
//Android	if (!picRxWin)
//Android		createRxViewer();
//Android	activate_mfsk_image_item(true);
	afcmetric = 0.0;
	datashreg = 1;
...
bool mfsk::check_picture_header(char c)
{
	...
	if (*p == '4') RXspp = 4;
	if (*p == '2') RXspp = 2;
	//Android: For sending signatures (particularly in greyscale)
	if (*p == '1') RXspp = 1;
	p++;
	if (!*p)
		return false;
...
void mfsk::recvpic(cmplx z)
{
	...
	int byte;
//Android	picf += arg( conj(prevz) * z) * samplerate / TWOPI;
	picf += (cmplx(prevz.real(), -prevz.imag()) * z).arg() * samplerate / TWOPI;
	prevz = z;
...
void mfsk::recvchar(int c)
{
	...
	if (check_picture_header(c) == true) {
		...
		//Android printf("symbolbit = %d, symbits = %d\n", symbolbit, symbits);
		switch (mode) {
			case MODE_MFSK16:
...
//Android		binmag = 2.0f * abs(bins[k]); // give harddecode() a vote in softdecode's decision.
			binmag = 2.0f * bins[k].mag(); // give harddecode() a vote in softdecode's decision.
		else
//Android		binmag = abs(bins[k]);
			binmag = bins[k].mag();
...
//Android	val = abs(pipe[j].vector[prev1symbol]);
...
//Android Not yet	if (staticburst || !progStatus.afconoff)
//Android		return;
//Android Not yet	if (metric < progStatus.sldrSquelchValue)
//Android		return;
	if (afcmetric < 3.0)
		return;
	if (currsymbol != prev1symbol)
...
//Android	z = conj(prevvector) * currvector;
	z = prevvector % currvector;

//Android	f = arg(z) * samplerate / TWOPI;
...
void mfsk::eval_s2n()
{
//Android	sig = abs(pipe[pipeptr].vector[currsymbol]);
//Android	noise = (numtones -1) * abs(pipe[pipeptr].vector[prev2symbol]);
...
//Android int mfsk::rx_process(const double *buf, int len)
int mfsk::rx_process(const short *buf, int len)
{
	cmplx z;
	cmplx* bins = 0;
...
//Android		z = cmplx( *buf, *buf );
		z = cmplx( (double)*buf, (double)*buf );
		buf++;
		hbfilt->run ( z, z );
...
//Android: made conditional to "SLOW CPU" as it is CPU intensive on older Android devices
...
double *ptr;
	...
	for (i = 0; i < len; i++) {
		//JD not yet		if (txstate == TX_STATE_PICTURE)
		...
		for (j = 0; j < TXspp; j++) {
			*ptr++ = cos(phaseacc);
			phaseacc += (TWOPI * f / samplerate) * 0.99;
...
//JD Not yet				put_status("Tx pic: start");
			...
			while (i < xmtbytes) {
				...
				if ( (100 * i / xmtbytes) % 2 == 0) {
					snprintf(mfskmsg, sizeof(mfskmsg),
							 "Tx pic: %3.1f%%",
							 (100.0f * i) / xmtbytes);
//JD not yet					put_status(mfskmsg);
				}
				i += blocklen;
			}
			flushtx(preamble);

//Android			REQ_FLUSH(GET_THREAD_ID());

			txstate = TX_STATE_DATA;
//JD not yet			put_status("Tx pic: done");
/*Android	FL_LOCK_E();
...
{
//JD not yet	load_image(s.c_str());
//JD not yet 	pic_TxSendColor();
}
